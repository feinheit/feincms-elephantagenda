...
if not 'facebook' in settings.INSTALLED_APPS:
...
class Event(EventFromFacebook):
    ...
    # custom fields:    
    categories = models.ManyToManyField(Category, blank=True, null=True, related_name="%(app_label)s_%(class)s_related")
    ...
    image = models.ImageField(upload_to='event_images/%Y/%m', blank=True, null=True,
                              max_length=200)
    max_places = models.PositiveSmallIntegerField(_(u'Anzahl Plätze'), blank=True, null=True) 
    # quick fix
    access_token = models.CharField(max_length=100, blank=True, help_text='for internal use only.')
    ...
    def get_from_facebook(self, graph=None, save=False):
        ...
        if getattr(graph, 'access_token', None):
            params = urllib.urlencode({'type': 'large', 'access_token': graph.access_token })
            url = 'https://graph.facebook.com/%s/picture/?%s' % (self.id, params)
            try:
